package com.yic3.lib.widget.indicator
...
class CuteIndicator @JvmOverloads constructor(
    ...
    init {
        ...
        mIndicatorSelectedColor =
            typedArray.getColor(R.styleable.CuteIndicator_IndicatorSelectedColor, Color.WHITE)
        mIndicatorWidth =
            typedArray.getDimension(R.styleable.CuteIndicator_IndicatorWidth, dp2px(5f))
        mIndicatorHeight =
            typedArray.getDimension(R.styleable.CuteIndicator_IndicatorHeight, dp2px(5f))
        mIndicatorSelectedWidth =
            typedArray.getDimension(R.styleable.CuteIndicator_IndicatorSelectedWidth, dp2px(20f))
        mIndicatorMargin =
            typedArray.getDimension(R.styleable.CuteIndicator_IndicatorMargin, dp2px(5f))
        mShowAnimation =
            typedArray.getBoolean(R.styleable.CuteIndicator_IndicatorShowAnimation, true)
    ...
    override fun onMeasure(widthMeasureSpec: Int, heightMeasureSpec: Int) {
        ...
        if (mIndicatorCount > 0) {
            val width =
                ((mIndicatorCount - 1) * (mIndicatorMargin + mIndicatorWidth) + mIndicatorSelectedWidth).toInt()
        ...
        if (position == (mIndicatorCount - 1) && positionOffset > 0f) {
            for (i in 0 until mIndicatorCount) {
                if (i == 0) {
                    left = 0f
                    right =
                        left + mIndicatorWidth + (mIndicatorSelectedWidth - mIndicatorWidth) * positionOffset
