...
class CircleIndicator @JvmOverloads constructor(
    ...
    init {
        ...
        mIndicatorColor =
            typedArray.getColor(R.styleable.CircleIndicator_ci_IndicatorColor, Color.GRAY)
        mIndicatorSelectedColor =
            typedArray.getColor(R.styleable.CircleIndicator_ci_IndicatorSelectedColor, Color.WHITE)
        mIndicatorWidth =
            typedArray.getDimension(R.styleable.CircleIndicator_ci_IndicatorWidth, dp2px(5f))
        mIndicatorMargin =
            typedArray.getDimension(R.styleable.CircleIndicator_ci_IndicatorMargin, dp2px(5f))
        mShowAnimation =
            typedArray.getBoolean(R.styleable.CircleIndicator_ci_IndicatorShowAnimation, true)
    ...
    override fun onMeasure(widthMeasureSpec: Int, heightMeasureSpec: Int) {
        ...
        if (mIndicatorCount > 0) {
            val width =
                ((mIndicatorCount - 1) * (mIndicatorMargin + mIndicatorWidth) + mIndicatorWidth)
    ...
    private fun drawIndicator(canvas: Canvas) {
        ...
        if (positionOffset == 0.0f) {
            canvas.drawCircle(
                startLeft + (startRight - startLeft) / 2,
                mIndicatorWidth / 2f,
                mIndicatorWidth / 2,
                mIndicatorPaint
            )
    ...
    override fun onPageScrolled(position: Int, positionOffset: Float, positionOffsetPixels: Int) {
        if (mShowAnimation) {
            this.position = position % mIndicatorCount
